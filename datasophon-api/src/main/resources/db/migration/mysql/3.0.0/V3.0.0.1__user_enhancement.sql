...
-- 添加上次登录时间字段
ALTER TABLE `t_ddh_user_info` 
ADD COLUMN `previous_login_time` DATETIME COMMENT '上次登录时间' AFTER `last_login_time`;

-- 添加用户头像字段（存储Base64编码的图片数据）
ALTER TABLE `t_ddh_user_info` 
ADD COLUMN `avatar` LONGTEXT COMMENT '用户头像（Base64编码）' AFTER `previous_login_time`;
