...
-- 修复ref_id字段类型问题（达梦数据库版本）
-- 将INT类型改为BIGINT以支持雪花算法生成的Long类型ID

-- 修改t_ddh_config_version_info表的ref_id字段类型
ALTER TABLE t_ddh_config_version_info MODIFY ref_id BIGINT NOT NULL; -- '关联对象ID（支持雪花算法生成的Long类型ID）'

-- 同时修改user_id字段类型（确保与雪花算法生成的用户ID兼容）
ALTER TABLE t_ddh_config_version_info MODIFY user_id BIGINT DEFAULT NULL; -- '用户ID（支持雪花算法生成的Long类型ID）'
