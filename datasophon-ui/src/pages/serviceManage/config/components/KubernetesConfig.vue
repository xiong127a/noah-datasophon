  ...
  <div class="kubernetes-config-container">
    ...
    <div class="page-header">
      <div class="header-content">
        <h2 class="title">{{ serviceName }} Kubernetes 配置</h2>
        <p class="subtitle">管理{{ serviceName }}服务的Kubernetes配置</p>
      </div>
    </div>
    ...
    <div class="kubernetes-dashboard-layout">
      ...
      <div class="sidebar-menu">
        <!-- 工作负载分组 -->
        <div class="menu-group" v-if="hasWorkloadData">
  ...
  computed: {
    hasWorkloadData() {
      return this.deployments.length > 0 || 
             this.statefulSets.length > 0 ||
             this.cronJobs.length > 0 ||
             this.daemonSets.length > 0 ||
             this.jobs.length > 0 ||
             this.pods.length > 0 ||
             this.replicaSets.length > 0 ||
             this.replicationControllers.length > 0;
    }
  },
  methods: {
    loadData() {
      console.log('KubernetesConfig loadData 被调用，加载Kubernetes仪表盘数据');
      if (this.serviceId) {
    ...
    async fetchConfigMaps() {
      try {
        const res = await this.$axiosGet(global.API.getKubernetesConfigMaps, {
    ...
    async fetchServices() {
      try {
        const res = await this.$axiosGet(global.API.getKubernetesServices, {
          clusterId: this.clusterId,
          serviceName: this.serviceName
        });
        if (res.code === 200) {
    ...
    async fetchPvcs() {
      try {
        const res = await this.$axiosGet(global.API.getKubernetesPvcs, {
          clusterId: this.clusterId,
          serviceName: this.serviceName
        });
        if (res.code === 200) {
    ...
    async fetchIngresses() {
      try {
        const res = await this.$axiosGet(global.API.getKubernetesIngresses, {
          clusterId: this.clusterId,
          serviceName: this.serviceName
        });
        if (res.code === 200) {
    ...
    async fetchIngressClasses() {
      try {
        const res = await this.$axiosGet(global.API.getKubernetesIngressClasses, {
          clusterId: this.clusterId,
          serviceName: this.serviceName
        });
        if (res.code === 200) {
    ...
    async fetchSecrets() {
      try {
        const res = await this.$axiosGet(global.API.getKubernetesSecrets, {
          clusterId: this.clusterId,
          serviceName: this.serviceName
        });
        if (res.code === 200) {
    ...
    async fetchstorage_classes() {
      try {
        const res = await this.$axiosGet(global.API.getKubernetesstorage_classes, {
          clusterId: this.clusterId,
          serviceName: this.serviceName
        });
        if (res.code === 200) {
    ...
    async fetchStatefulSets() {
      try {
        const res = await this.$axiosGet(global.API.getKubernetesStatefulSets, {
          clusterId: this.clusterId,
          serviceName: this.serviceName
        });
        if (res.code === 200) {
...
.kubernetes-table :deep(.ant-table-thead > tr > th) {
  background-color: #f8f9fa;
  font-weight: 600;
  color: #586069;
  border-bottom: 1px solid #e9ecef;
...
.kubernetes-table :deep(.ant-table-tbody > tr > td) {
  border-bottom: 1px solid #f1f3f5;
}

.kubernetes-table :deep(.ant-table-tbody > tr:hover > td) {
  background-color: #f8f9fa;
}
