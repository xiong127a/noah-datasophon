          ...
          >
            <template slot="name" slot-scope="text, record">
              <div style="display: flex; align-items: center; line-height: normal;">
                <div class="name-cell">
                  <span class="rc-name" :title="record.objectMeta?.name || '-'">
...
<script>
export default {
  name: 'ReplicationControllersDashboard',
  props: {
    clusterId: {
      type: Number,
      required: true
  ...
  methods: {
    ...
    async fetchReplicationControllers() {
      this.loading = true;
      console.log('获取ReplicationControllers，命名空间:', this.selectedNamespace);
      ...
      try {
        ...
        // 使用全局API对象中定义的getKubernetesReplicationControllers接口
        const res = await this.$axiosGet(global.API.getKubernetesReplicationControllers, params);
        ...
        if (res.code === 200 && res.data) { 
          ...
          this.replicationControllers = this.replicationControllers.map(rc => {
            return {
              ...
              containerImages: rc.containerImages || [],
              initContainerImages: rc.initContainerImages || [],
              // 确保podInfo存在
              podInfo: rc.podInfo || {
                current: 0,
                desired: 0,
              ...
              // 确保typeMeta存在并正确设置kind为replicationcontroller
              typeMeta: {
                ...rc.typeMeta,
