      ...
      getExampleList (flag) {
        ...
        this.$axiosPost(global.API.instanceList, params).then((res) => {
          ...
          if (res.code === 200) {
            ...
            if(this.pagination.current==1 && this.pagination.pageSize==10){
                this.page1 = res.data;
            } 
           
            //塞入假数据
            const dataNodeItem = res.data[0];
            let datanodeTotal = 1000;
            let allData = []
            const dataNodeList = []
            if(this.serviceName =='HDFS'){
              datanodeTotal = 1000;
              for(let it =1;it<datanodeTotal;it++){
                dataNodeList.push({
                  ...dataNodeItem,
                  hostname:'ndp'+it,
                  id: it,
            ...
            if(this.pagination.current>1 && this.pagination.pageSize==10){
                allData = this.page1.concat(res.data)
                allData = allData.concat(dataNodeList)
            }else{
                allData = res.data.concat(dataNodeList)
            }
           
            this.dataSources = allData;
            this.pagination.total = res.total+datanodeTotal;
          }
        });
      },
      pollingSearch () {
