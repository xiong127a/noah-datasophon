  ...
  <div style="padding-top: 10px">
    <a-form :label-col="labelCol" :wrapper-col="wrapperCol" :form="form" class="p0-32-10-32 form-content"
      ...
      <a-form-item label="组件配置">
        ...
        <div class="tabs">
          <div :class="item.active == true ? 'active' : 'tabItem'" v-for="(item, index) in tabsList" :key="index"
            @click="changeTab(item.key, item.active)">{{ item.name }}
          </div>
        </div>
        <div class="module">
          <div class="titleTop">
          ...
          <div class="moduleItems" v-show="resourceList.hbaseResourceList.length > 0">
            ...
            <!-- <div class="clearDom">
              <a-icon type="delete" class="clearIcon" @click="toClear('hbaseResourceList')"></a-icon>
            </div> -->
          </div>
          <div class="moduleItems" v-show="resourceList.hiveResourceList.length > 0">
            ...
            <!-- <div class="clearDom">
              <a-icon type="delete" class="clearIcon" @click="toClear('hiveResourceList')"></a-icon>
            </div> -->
          </div>
...
export default {
  ...
  data () {
    return {
      ...
      tabsList: [{ name: 'HDFS', key: 'hdfsResourceList', active: false }, { name: 'YARN', key: 'yarnResourceList', active: false }, { name: 'HBASE', key: 'hbaseResourceList', active: false }, { name: 'HIVE', key: 'hiveResourceList', active: false }, { name: 'KAFKA', key: 'kafkaResourceList', active: false },
      ],
      resourceListINIT: {
        "hdfsResourceList": [
          {
  ...
  watch: {},
  methods: {
    changeTab (type, active) {
      this.tabsList.forEach((e) => {
        if (type == e.key) {
          e.active = !e.active
          if (e.active == 0 || e.active == false) {
    ...
    echoUSer () {
      if (JSON.stringify(this.detail) !== "{}") {
        ...
        let arr = keys.filter(item => resourceNames.includes(item) && this.detail[item].length > 0)
        this.tabsList.forEach(j => {
          if (arr.includes(j.key)) {
            j.active = true
          }
        })
...
.module .moduleItems .right .title :nth-last-child(1) {
  display: inline-block;
  flex: .15;
  text-indent: 20px;
}
