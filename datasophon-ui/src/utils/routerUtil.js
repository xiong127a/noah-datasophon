...
function loadRoutes(routesConfig, clusterRoutes) {
  ...
  if (menuRoutes) {
    ...
    const menuData = localStorage.getItem('menuData') ? JSON.parse(localStorage.getItem('menuData')) : []
    if (isCluster === 'isCluster' && menuData) {
      store.dispatch('setting/getRunningClusterList') // 这个刷新页面时候去定时刷新服务管理这个列表
...
export {parseRoutes, loadRoutes, formatAuthority, getI18nKey, loadGuards, deepMergeRoutes, formatRoutes, setAppOptions, setDynamicRouter}
