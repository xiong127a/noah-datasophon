...
import { DIALOG_STYLES, BUTTON_STYLES, TABLE_STYLES, HOST_STYLES, BADGE_STYLES } from '../common/shared-styles'
import type { 
  KubernetesStep1Data,
  KubernetesStep2DialogProps,
  KubernetesHost,
...
/**
 * Kubernetes集群主机校验步骤 - 专用于Kubernetes集群
 * 作者：任相鹏
 * 邮箱：635887935@qq.com
 * 日期：2024-01-20
 */

const KubernetesClusterStep2Dialog: React.FC<KubernetesStep2DialogProps> = ({
  open,
  onOpenChange,
  cluster,
  step1Data,
  ...
  // 计算可选择的主机数量和状态
  const selectableCount = filteredData.filter(host => 
    ManagementStatusUtil.isUnmanaged(host.managementStatus || ManagementStatus.UNMANAGED)
  ).length
  ...
  // 发现K8S主机（基于Step1配置）
  const getK8SEnvironmentList = async (showLoading = true) => {
    ...
    try {
      ...
      const response = await clusterApi.unifiedHost.discoverFromStep1(k8sConfig, { headers })
      ...
      if (res.code === 200) {
        ...
        console.error('获取K8S主机列表失败:', res.msg || '未知错误')
        setDataSource([])
        toast.error('获取K8S主机列表失败')
      }

      firstDataLoadedRef.current = true
    } catch (error) {
      console.error('请求K8S主机失败:', error)
      toast.error('获取K8S主机列表失败')
    } finally {
      setLoading(false)
  ...
  // 创建稳定的函数引用
  const getK8SEnvironmentListRef = useRef(getK8SEnvironmentList)
  getK8SEnvironmentListRef.current = getK8SEnvironmentList
  ...
  const saveK8sConfigAndHosts = async () => {
    try {
      ...
      if (successfulHosts && successfulHosts.length > 0) {
        console.log('K8S主机已通过unifiedHost API处理:', successfulHosts)
      }
      
      toast.success('K8S配置保存成功')
  ...
  const handleNext = async () => {
    ...
    try {
      // 检查K8S主机校验是否完成
      const checkResult = await k8sHostCheckCompleted()
      
      if (!checkResult.hostCheckCompleted) {
        toast.warning(checkResult.data || '存在未校验成功的K8S主机')
        return
      }
      ...
      toast.success('K8S Step2 校验完成，进入下一步')
      ...
      console.error('K8S下一步处理失败:', error)
      toast.error('保存K8S配置失败，请重试')
    } finally {
      setLoading(false)
    }
  }
  ...
  // K8S主机环境校验是否完成
  const k8sHostCheckCompleted = async (): Promise<{ hostCheckCompleted: boolean; data: string }> => {
    try {
      ...
      if (result.code === 200 && result.data) {
        ...
        return {
          ...
          data: validationResult.message || 'K8S校验完成'
        }
      } else {
        return {
          hostCheckCompleted: false,
          data: result.msg || 'K8S校验失败，请重试'
        }
      }
    } catch (error) {
      console.error('调用K8S后端校验接口失败:', error)
      return {
        hostCheckCompleted: false,
        data: '无法连接后端进行K8S校验，请检查网络连接'
      }
    }
  }
  ...
  const refreshK8sHosts = () => {
    ...
    getK8SEnvironmentListRef.current(true)
  }
  ...
  useEffect(() => {
    if (open && clusterId) {
      getK8SEnvironmentListRef.current()
    }
  }, [open, clusterId])
                      ...
                      <CardContent className="pt-0 flex-1 flex flex-col min-h-0">
                        {loading ? (
                          ...
                          <div className="flex flex-col items-center justify-center h-64 text-gray-500">
                            ...
                            <div className="text-lg mb-2">暂无K8S主机数据</div>
                            <div className="text-sm">请检查Kubernetes集群配置或点击重新校验</div>
                          </div>
                        ) : filteredData.length === 0 ? (
                          <div className="flex flex-col items-center justify-center h-64 text-gray-500">
                            <div className="text-4xl mb-4">🔍</div>
                            <div className="text-lg mb-2">没有找到匹配的K8S主机</div>
...
export default KubernetesClusterStep2Dialog
