...
public class K8sFlinkOperatorHandlerStrategy extends K8sAbstractHandlerStrategy implements K8sServiceRoleStrategy {

    private static final String CRD_DIR = "k8s/templates/FLINKOPERATOR/crd";

    public K8sFlinkOperatorHandlerStrategy(String serviceName, String serviceRoleName) {
    ...
    private void submitCRDs(KubernetesClient client) {
        ...
        try {
            ...
            if ("file".equals(resource.getProtocol())) {
                File crdDir = new File(URLDecoder.decode(resource.getFile(), StandardCharsets.UTF_8.name()));
                if (crdDir.isDirectory()) {
                    // 扫描目录下所有YAML文件
                    File[] crdFiles = crdDir.listFiles((dir, name) -> 
