        ...
        if (command.getCommandType().equals(CommandType.INSTALL_SERVICE)) {
            ...
            if (startResult.getExecResult()) {
                try (KubernetesClient kubeClient = KubeUtil.getKubeClientByConfig(command.getKubeConfig())) {
                    ...
                    StringBuilder checkCmdBuilder = new StringBuilder();
                    ...
                    for (int i = startPodIndex; i <= endPodIndex; i++) {
                        ...
                        // 为每个节点生成连接检测命令
                        String checkCmd = generateConnectionCheckCommand(beHost, 9050);

                        // 执行连接检测命令
                        logger.info("检测BE节点 {} 连接是否可用", beHost);
                        ...
                        if (!checkResult.getExecResult()) {
                            logger.error("BE节点 {} 连接检测失败，跳过添加此节点", beHost);
                            logger.error("错误信息: {}", checkResult.getExecErrOut());
                            continue;
                        }

                        logger.info("BE节点 {} 连接检测成功，准备添加节点", beHost);

                        // 使用单引号而非双引号，避免多层转义问题
                        String singleCmd = String.format(
                                "mysql -h127.0.0.1 -P9030 -uroot --connect-timeout=10 -e 'ALTER SYSTEM ADD BACKEND \"%s\"'",
                                beHostPort);
