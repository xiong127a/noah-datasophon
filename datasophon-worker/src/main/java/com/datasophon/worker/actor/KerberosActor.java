...
public class KerberosActor extends AbstractActor {
    ...
    public Receive createReceive() {
        return ReceiveBuilder.create()
                .match(GenerateKeytabFileCommand.class, command -> {
                    ExecResult execResult = new ExecResult();
                    String keytabFilePath = "/etc/security/keytab/" + command.getHostname() + Constants.SLASH
                            + command.getKeytabName();
                    logger.info("find keytab file {}", keytabFilePath);
                    if (!FileUtil.exist(keytabFilePath)) {
                        FileUtil.mkParentDirs(keytabFilePath);
                        String addprinc = "kadmin -padmin/admin -wadmin -q \"addprinc -randkey "
