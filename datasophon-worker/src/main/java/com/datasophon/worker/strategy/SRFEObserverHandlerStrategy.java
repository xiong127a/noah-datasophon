...
public class SRFEObserverHandlerStrategy extends AbstractHandlerStrategy implements ServiceRoleStrategy {
    ...
    public SRFEObserverHandlerStrategy(String serviceName, String serviceRoleName) {
    ...
    public ExecResult handler(ServiceRoleOperateCommand command) {
        ...
        if (command.getCommandType() == CommandType.INSTALL_SERVICE) {
            ...
            if (startResult.getExecResult()) {
                logger.info("StarRocks FE observer start success, will be added to cluster automatically by API");
            } else {
                logger.error("StarRocks FE observer start failed");
            }
        } else {
