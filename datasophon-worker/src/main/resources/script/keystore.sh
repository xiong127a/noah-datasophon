...
host=$1
echo ${hostname}
keytool -keystore /etc/security/keytab/keystore -alias localhost -validity 9999 -importpass -keypass admin123  -importpass -storepass admin123 -genkey -keyalg RSA -keysize 2048 -dname "CN=${hostname}, OU=${hostname}, O=${hostname}, L=hefei, ST=hefei, C=CN"
