...
public class SshConnectivityCheckPlugin implements HostCheckerPlugin {
     ...
     * @return 上传是否成功
     */
    public boolean uploadFile(String hostIp, int port, String username, String password, 
                             String localFilePath, String remoteFilePath) {
        log.info("【SSH插件】上传文件: {}@{}:{} {} -> {}", username, hostIp, port, localFilePath, remoteFilePath);
        
        if (sshPoolManager == null) {
            log.error("【SSH插件】SSH连接池管理器未初始化");
            return false;
        }
        
        try {
            // 使用连接池管理器上传文件
            return sshPoolManager.uploadFile(hostIp, port, username, password, localFilePath, remoteFilePath);
            
        } catch (Exception e) {
            log.error("【SSH插件】文件上传失败: {}@{}:{} {} -> {}, 错误: {}", 
                    username, hostIp, port, localFilePath, remoteFilePath, e.getMessage(), e);
            return false;
        }
    }
     ...
     * @return 上传是否成功
     */
    public boolean uploadFileFromStream(String hostIp, int port, String username, String password, 
                                       java.io.InputStream inputStream, String remoteFilePath) {
        log.info("【SSH插件】上传文件流: {}@{}:{} -> {}", username, hostIp, port, remoteFilePath);
        ...
        try {
            // 使用连接池管理器上传文件流
            return sshPoolManager.uploadFileFromStream(hostIp, port, username, password, inputStream, remoteFilePath);
            
        } catch (Exception e) {
            log.error("【SSH插件】文件流上传失败: {}@{}:{} -> {}, 错误: {}", 
                    username, hostIp, port, remoteFilePath, e.getMessage(), e);
            return false;
        }
    }
     ...
     * @return 下载是否成功
     */
    public boolean downloadFile(String hostIp, int port, String username, String password, 
                               String remoteFilePath, String localFilePath) {
        log.info("【SSH插件】下载文件: {}@{}:{} {} -> {}", username, hostIp, port, remoteFilePath, localFilePath);
        ...
        try {
            // 使用连接池管理器下载文件
            return sshPoolManager.downloadFile(hostIp, port, username, password, remoteFilePath, localFilePath);
            
        } catch (Exception e) {
            log.error("【SSH插件】文件下载失败: {}@{}:{} {} -> {}, 错误: {}", 
                    username, hostIp, port, remoteFilePath, localFilePath, e.getMessage(), e);
            return false;
        }
    }
     ...
     * @return 创建是否成功
     */
    public boolean createDirectory(String hostIp, int port, String username, String password, String remotePath) {
        log.info("【SSH插件】创建目录: {}@{}:{} -> {}", username, hostIp, port, remotePath);
        ...
        try {
            // 使用连接池管理器创建目录
            return sshPoolManager.createDirectory(hostIp, port, username, password, remotePath);
            
        } catch (Exception e) {
            log.error("【SSH插件】目录创建失败: {}@{}:{} -> {}, 错误: {}", 
                    username, hostIp, port, remotePath, e.getMessage(), e);
            return false;
        }
    }
     ...
     * @return 删除是否成功
     */
    public boolean deleteFile(String hostIp, int port, String username, String password, String remoteFilePath) {
        log.info("【SSH插件】删除文件: {}@{}:{} -> {}", username, hostIp, port, remoteFilePath);
        ...
        try {
            // 使用连接池管理器删除文件
            return sshPoolManager.deleteFile(hostIp, port, username, password, remoteFilePath);
            
        } catch (Exception e) {
            log.error("【SSH插件】文件删除失败: {}@{}:{} -> {}, 错误: {}", 
                    username, hostIp, port, remoteFilePath, e.getMessage(), e);
            return false;
        }
    }
     ...
     * @return 路径是否存在
     */
    public boolean checkPathExists(String hostIp, int port, String username, String password, String remotePath) {
        log.debug("【SSH插件】检查路径存在: {}@{}:{} -> {}", username, hostIp, port, remotePath);
        ...
        try {
            // 使用连接池管理器检查路径
            return sshPoolManager.checkPathExists(hostIp, port, username, password, remotePath);
            
        } catch (Exception e) {
            log.error("【SSH插件】路径检查失败: {}@{}:{} -> {}, 错误: {}", 
                    username, hostIp, port, remotePath, e.getMessage(), e);
            return false;
        }
    }
     ...
     * @return 创建是否成功
     */
    public boolean createFile(String hostIp, int port, String username, String password, String remoteFilePath) {
        log.info("【SSH插件】创建文件: {}@{}:{} -> {}", username, hostIp, port, remoteFilePath);
        ...
        try {
            // 使用连接池管理器创建文件
            return sshPoolManager.createFile(hostIp, port, username, password, remoteFilePath);
            
        } catch (Exception e) {
            log.error("【SSH插件】文件创建失败: {}@{}:{} -> {}, 错误: {}", 
                    username, hostIp, port, remoteFilePath, e.getMessage(), e);
            return false;
        }
    }
