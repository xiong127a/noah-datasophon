...
public class FEObserverHandlerStartegy implements ServiceRoleStrategy {
    ...
    public void handlerServiceRoleCheck(ClusterServiceRoleInstanceDTO roleInstanceDto, Map<String, ClusterServiceRoleInstanceDTO> map) {
        ...
        String feMaster = globalVariables.get("${feMaster}");
        if (roleInstanceDto.hostname().equals(feMaster) && ServiceRoleState.RUNNING.getValue() == roleInstanceDto.serviceRoleState()) {
            try {
    ...
    private void resolveProcInfoAlert(String serviceRoleName, List<ProcInfo> frontends, Map<String, ClusterServiceRoleInstanceDTO> map) {
