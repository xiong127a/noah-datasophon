    ...
    public boolean handle(ClientSession session, HostInfo hostInfo) throws UnknownHostException {
        ...
        if (StringUtils.isBlank(updateCommonPropertiesResult) || "failed".equals(updateCommonPropertiesResult)) {
            ...
            if (useSystemd) {
                ...
                boolean createResult = MinaUtils.createSystemdServiceForDebian(session, serviceDir + "/datasophon-worker", installPath);
            ...
            if (useSystemd) {
                ...
                result = MinaUtils.safeExecCommand(session, installPath + "/datasophon-worker/bin/datasophon-worker.sh start worker");
