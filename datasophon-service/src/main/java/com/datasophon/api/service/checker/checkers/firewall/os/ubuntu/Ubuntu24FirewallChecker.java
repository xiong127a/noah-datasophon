...
/**
 * Ubuntu 24.04防火墙检查器
 * 专门处理Ubuntu 24.04系统的防火墙检查和修复
 */
public class Ubuntu24FirewallChecker extends UbuntuFirewallChecker {

    private static final Logger log = LoggerFactory.getLogger(Ubuntu24FirewallChecker.class);

    @Override
    public CheckItem check(HostInfo hostInfo, CheckItem checkItem, CheckLogger cacheLog) throws InterruptedException {
        cacheLog.info("==== Ubuntu 24.04防火墙检查开始 ====");
    ...
    public boolean fix(HostInfo hostInfo, CheckItem checkItem, CheckLogger cacheLog) throws InterruptedException {
        cacheLog.info("==== Ubuntu 24.04防火墙修复开始 ====");
        ...
        if (firewallType == FirewallType.UFW) {
            // Ubuntu 24.04默认使用ufw
            cacheLog.info("检测到Ubuntu 24.04使用ufw防火墙(默认)，正在修复...");
            return fixUfw(session, checkItem, cacheLog);
        } else if (firewallType == FirewallType.FIREWALLD) {
            cacheLog.info("检测到Ubuntu 24.04使用firewalld防火墙(非默认)，正在修复...");
            return fixFirewalld(session, checkItem, cacheLog);
        } else if (firewallType == FirewallType.IPTABLES) {
            cacheLog.info("检测到Ubuntu 24.04使用iptables防火墙，正在修复...");
     ...
     * 检查Ubuntu 24.04特有的服务
     */
    private void checkUbuntu24SpecificServices(ClientSession session, CheckLogger cacheLog)
            throws InterruptedException {
        // 调用父类方法
        checkUbuntuSpecificServices(session, cacheLog);

        // 检查是否启用了nftables（Ubuntu 24.04默认使用nftables）
        ...
        if (nftablesResult.isSuccess() && !nftablesResult.getOutput().contains("Not Found")) {
            cacheLog.info("检测到nftables工具，Ubuntu 24.04默认使用nftables作为防火墙后端");
        ...
        // 检查Ubuntu 24.04特有的网络管理服务 - systemd-networkd
        ...
        // 检查Ubuntu 24.04特有的安全特性 - secureboot
        ...
        // 检查Ubuntu 24.04特有的容器服务 - podman/docker
        ...
        if (podmanResult.isSuccess() && !podmanResult.getOutput().contains("Not Found")) {
            cacheLog.info("检测到podman容器工具，可能会影响网络配置和防火墙规则");
        }

        // 检查systemd的版本（Ubuntu 24.04使用更新的systemd版本）
        CommandResult systemdVersionResult = execCommand(session, "systemctl --version | head -n 1", cacheLog);
        if (systemdVersionResult.isSuccess()) {
            cacheLog.info("systemd版本: %s", systemdVersionResult.getOutput().trim());
        }
    }
}