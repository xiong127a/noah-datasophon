 ...
 */
@Service
public class AsyncCheckServiceImpl {
    private static final Logger logger = LoggerFactory.getLogger(AsyncCheckServiceImpl.class);
    ...
    private CheckItem doCheck(Integer clusterId, HostInfo hostInfo, CheckItem checkItem) {
        // 获取检查项代码，可能为null
        String itemCodeStr = checkItem.getItemCode();
        if (itemCodeStr == null) {
            logger.error("检查项代码为空: {}", checkItem.getItemName());
            checkItem.setStatus(CheckItem.Status.FAILED);
            checkItem.setMessage("检查项代码为空");
            return checkItem;
        }
        
        // 使用String版本的getChecker方法
        ItemChecker checker = itemCheckerFactory.getChecker(itemCodeStr);
        
        if (checker == null) {
            logger.error("找不到检查项对应的检查器: {}", itemCodeStr);
            checkItem.setStatus(CheckItem.Status.FAILED);
