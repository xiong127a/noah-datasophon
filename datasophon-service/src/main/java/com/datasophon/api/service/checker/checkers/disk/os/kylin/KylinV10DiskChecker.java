...
/**
 * Kylin V10磁盘检查器实现
 * 专用于Kylin V10 Linux发行版
 */
public class KylinV10DiskChecker extends KylinDiskChecker {

    private static final Logger log = LoggerFactory.getLogger(KylinV10DiskChecker.class);

    @Override
    public void provideCleanupSuggestions(CheckLogger cacheLog) {
        cacheLog.warn("银河麒麟V10系统磁盘空间不足，建议以下清理措施:");
        ...
        cacheLog.warn("6. 使用磁盘分析工具: sudo yum install -y ncdu && sudo ncdu /");
        cacheLog.warn("7. 清理Docker镜像和容器(如果安装了Docker): sudo docker system prune -a");
        cacheLog.warn("8. 考虑扩展磁盘分区或挂载新磁盘");
    }
}