...
public class KubernetesTrinoHandlerStrategy extends KubernetesAbstractHandlerStrategy
    ...
    public void getConfig(Integer clusterId, String namespace, List<ServiceConfig> list) {
        ...
        for (ServiceConfig config : list) {
            ...
            if ("discovery.uri".equals(name)) {
                try {
                    String value = (String) config.getValue();// 获取当前配置值
                    config.setValue(
                            FileUtils.replaceHost(value, "trino-trinocoordinator.datasophon.svc.cluster.local")); // 去掉最后一个逗号
                } catch (Exception e) {
                    // 忽略错误
                }
            }
