...
public class LogEntry implements Serializable {
    ...
    public String toColoredHtml() {
        ...
        sb.append(String.format(
                "<div class=\"log-entry\" style=\"margin-bottom: 8px; padding: 8px 12px; background-color: %s; border-left: 3px solid %s;\">",
                bgColor, borderColor));
        ...
        // 日志级别
        sb.append(String.format(
                "<span class=\"log-level\" style=\"color: %s; font-weight: 500; padding: 1px 6px; border-radius: 2px; background-color: %s; font-size: 12px;\">%s</span>",
                levelColor, level.getBackgroundColor(), level.name()));
        // 线程名
        if (threadName != null) {
            sb.append(String.format("<span class=\"log-thread\" style=\"color: #8c8c8c; font-size: 12px;\">[%s]</span>",
                    threadName));
        }
        sb.append("</div>");
        ...
        sb.append(String.format("<div class=\"log-content\" style=\"color: %s;\">%s</div>", levelColor,
                formattedContent));
